...
def build_agent(tmp_path, outputs, **kwargs):
    ...
    store = SessionStore(tmp_path / ".repofox" / "sessions")
...
def test_agent_only_stores_reusable_epistemic_notes(tmp_path):
    ...
    resumed = RepoFoxAgent.from_session(
...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".repofox").mkdir(exist_ok=True)
    (tmp_path / ".git").mkdir(exist_ok=True)
    (tmp_path / "hello.txt").write_text("hi\n", encoding="utf-8")

    result = agent.run_tool("list_files", {})

    assert ".repofox" not in result
...
def test_welcome_screen_keeps_box_shape_for_long_paths(tmp_path):
    ...
    assert "MINI CODING AGENT" not in welcome
    assert "RepoFox" in welcome
    assert "local repo agent" in welcome
...
def test_ollama_client_streams_chunks_when_handler_is_provided():
    captured = {}
    streamed = []
    ...
    class FakeResponse:
        def __init__(self):
            self.lines = iter(
                [
                    b'{"response":"<final>Hello","done":false}\n',
                    b'{"response":" world</final>","done":false}\n',
    ...
    assert streamed == ["<final>Hello", " world</final>"]
    assert captured["url"] == "http://127.0.0.1:11434/api/generate"
    assert captured["timeout"] == 30
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        ...
        ), patch("repofox.cli.OpenAICompatibleModelClient") as mock_openai:
...
def test_build_agent_uses_anthropic_provider_and_openai_key_fallback(tmp_path):
    ...
    with patch.dict(
        ...
        ), patch("repofox.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("repofox.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_successful_run_persists_run_artifacts_and_stop_reason(tmp_path):
    ...
    runs_root = tmp_path / ".repofox" / "runs"
...
def test_agent_streams_only_final_answer_content(tmp_path):
    (tmp_path / "hello.txt").write_text("alpha\nbeta\n", encoding="utf-8")
    workspace = build_workspace(tmp_path)
    store = SessionStore(tmp_path / ".repofox" / "sessions")
    ...
    class StreamingModelClient:
        ...
        def complete(self, prompt, max_new_tokens, **kwargs):
            ...
            if stream_handler:
                ...
                stream_handler(output[:midpoint])
                stream_handler(output[midpoint:])
            return output
    ...
    streamed = []
    answer = agent.ask("Inspect hello.txt", stream_final=streamed.append)
...
def test_agent_emits_status_updates_for_model_and_tool_steps(tmp_path):
    ...
    statuses = []
    answer = agent.ask("Inspect hello.txt", status_update=statuses.append)

    assert answer == "Finished."
    assert [item["event"] for item in statuses] == [
        "thinking",
        "tool_started",
        "tool_finished",
        "thinking",
    ]
    assert statuses[1]["name"] == "read_file"
    assert statuses[2]["name"] == "read_file"


def test_status_printer_renders_human_readable_lines():
    buffer = io.StringIO()
    ui = repofox_cli.ConsoleUI(buffer, tick_interval=1.0, time_fn=lambda: 10.0)
    printer = repofox_cli.StatusPrinter(ui)

    printer.emit({"event": "thinking"})
    printer.emit({"event": "tool_started", "name": "read_file", "args": {"path": "README.md", "start": 1}})
    printer.emit({"event": "tool_finished", "name": "read_file", "duration_ms": 42})
    printer.close()
...
def test_thinking_label_includes_elapsed_seconds():
    assert repofox_cli._thinking_label(1.26, frame="/") == "Thinking / 1.3s"


def test_tool_arg_summary_uses_only_safe_args():
    summary = repofox_cli._tool_arg_summary(
        {
            "path": "README.md",
    ...
    assert summary == "path=README.md pattern=hello"


def test_sse_event_formats_standard_event_stream_frame():
    frame = repofox_cli._sse_event("final_delta", {"text": "你好"})

    assert frame == 'event: final_delta\ndata: {"text": "你好"}\n\n'


def test_run_agent_sse_emits_runtime_events(tmp_path):
    (tmp_path / "hello.txt").write_text("alpha\nbeta\n", encoding="utf-8")
    agent = build_agent(
        tmp_path,
        [
            '<tool>{"name":"read_file","args":{"path":"hello.txt","start":1,"end":2,"content":"hidden"}}</tool>',
            "<final>Finished.</final>",
        ],
    )
    events = []

    final = repofox_cli._run_agent_sse(
        agent,
        "Inspect hello.txt",
    ...
    assert names[-2:] == ["final_answer", "done"]

    tool_started = next(data for event, data in events if event == "tool_started")
    assert tool_started["name"] == "read_file"
    assert tool_started["args"] == {"path": "hello.txt", "start": 1, "end": 2}
    assert "content" not in tool_started["args"]

    final_delta = next(data for event, data in events if event == "final_delta")
    assert final_delta == {"text": "Finished."}
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .repofox/.\n"
    ...
    index_path = tmp_path / ".repofox" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".repofox" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".repofox" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .repofox/.",
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".repofox" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".repofox" / "memory" / "topics" / "dependency-facts.md"
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert RepoFoxAgent is not None
    assert OllamaModelClient is not None
    assert SessionStore is not None
    assert WorkspaceContext is not None
    assert Path(repofox_pkg.__file__).as_posix().endswith("/repofox/__init__.py")
...
def test_package_import_surface_includes_cli_entrypoints():
    assert callable(repofox_pkg.main)
    assert callable(repofox_pkg.build_agent)
    assert callable(repofox_pkg.build_arg_parser)


def test_module_execution_help_works():
    result = subprocess.run(
        [sys.executable, "-m", "repofox", "--help"],
        capture_output=True,
        text=True,
    )
